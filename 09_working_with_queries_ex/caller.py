...
def show_hard_dungeons() -> str:
    hard_level_dungeons = Dungeon.objects.filter(difficulty='Hard').order_by('-location')

    return '\n'.join([
        f"{d.name} is guarded by {d.boss_name} "
        f"who has {d.boss_health} health points!"
        for d in hard_level_dungeons
    ])
...
def show_workouts() -> str:
    cal_cross_workouts = Workout.objects.filter(workout_type__in=['Calisthenics', 'CrossFit'])

    return '\n'.join([
        f"{w.name} from {w.workout_type} "
        f"type has {w.difficulty} difficulty!"
        for w in cal_cross_workouts
    ])
