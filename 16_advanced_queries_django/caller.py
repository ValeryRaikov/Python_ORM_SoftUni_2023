...
def ordered_products_per_customer() -> str:
    # total_ordered_products = Order.objects.all().order_by('id')

    # With prefetched_related it is faster and better:
...
def filter_products() -> str:
    ...
    return '\n'.join([
        f"{product.name}: {product.price}lv." for product in products
    ])
...
def give_discount() -> str:
    ...
    query = query = Q(is_available=True) & Q(price__gt=3)

    products = Product.objects.filter(query).update(price=reduction)

    total_available_products = Product.objects.filter(is_available=True).order_by('-price', 'name')

    return '\n'.join([
        f"{product.name}: {product.price}lv." for product in total_available_products
    ])
